...
import { userRegist } from "../../../redux";
import { useDispatch, useSelector } from "react-redux";
import { Link, useNavigate } from "react-router-dom";
const FormRegisterUser = () => {
  const dispatch = useDispatch();
  const { data, loading, error } = useSelector((state) => state.userRegist);
  ...
  const handleSubmit = (e) => {
    ...
    // Dispatch the userRegist action with form data
    dispatch(
      userRegist(form, (status) => {
        if (status === 201) {
